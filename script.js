...
function updateLeaderboard() {
  const placementContainer = document.querySelector("div[data-container-id='placements']");
  ...
  for(let i = 0; i < leaderboard.length; i++) {
    ...
    <p class='flex-1 text-right'>${leaderboard[i].score} (${formatTime(leaderboard[i].time)})</p>
...
function toggleLeaderboard() {
  const leaderboardContainer = document.querySelector("div[data-section-id='leaderboard']")
...
class Question {
  ...
  generateQuestion() {
    do{
      ...
      (this.problem.operator == '/' && (this.problem.operand1 == 0 || this.problem.operand2 == 0)) || (this.problem.operator == "%" && this.problem.operand1 == 0  && this.operand2 == 0)
    );
...
generateQuiz() {
  const quizContainer = document.querySelector("div[data-container-id='quiz']");
